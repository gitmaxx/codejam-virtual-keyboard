const Keyboard = {
  elements: {
    ...
    init(wrp,out) {
        ...
        this._createKeys();
        this._setKeys(this.layouts[this.properties.lang].nocaps);
        this.elements.keysContainer.addEventListener("kbd", (event)=>{
            let z=this.hardKeys.indexOf(event.detail.code);
            let key=this.elements.keys[z];
            let caps= this.properties.capsLock ? "caps":"nocaps";
            let keyName=this.layouts[this.properties.lang][caps][z];
            ...
            switch (keyName) {
                ...
                break;
                case "Ctrl":
                    if (event.detail.alt) {this._switchLang()};
                break;
                case "Alt":
                    if (event.detail.ctrl) {this._switchLang()};
                break;
                case "Win":
                break;
                default:
        ...
        this.elements.keysContainer.addEventListener("kbdUp", ()=>{
        let z=this.hardKeys.indexOf(event.detail.code);
        let key=this.elements.keys[z];
        let caps= this.properties.capsLock ? "caps":"nocaps";
        let keyName=this.layouts[this.properties.lang][caps][z];
        if (keyName != "Caps Lock")  {setTimeout(function(){ key.classList.remove("keyboard--key-active");},200);}
        if (keyName==="Shift") {this._toggleCapsLock();};
        });
        this.elements.keysContainer.addEventListener("click", ()=>{
            if(event.target != this.elements.keysContainer) {
            let key=event.target;
            let index=this.elements.keys.indexOf(key);
            let caps= this.properties.capsLock ? "caps":"nocaps";
            let keyName=this.layouts[this.properties.lang][caps][index];
            if (keyName != "Caps Lock") {key.classList.add("keyboard--key-active");}
            switch (keyName) {
                case "Backspace":
                    this.properties.value = this.properties.value.slice(0,-1);
            ...
            if (keyName != "Caps Lock") {setTimeout(function(){ key.classList.remove("keyboard--key-active");},300);}
            this._triggerEvent("oninput");
        }});
    },
    ...
    _toggleCapsLock() {
        ...
        let currentLayout = this.properties.capsLock ? this.layouts[this.properties.lang].caps : this.layouts[this.properties.lang].nocaps;
        this._setKeys(currentLayout);
    },
...
window.addEventListener("DOMContentLoaded", ()=>{
    ...
    document.addEventListener("keydown", (event)=>{
        document.querySelector(".keyboard").dispatchEvent(new CustomEvent("kbd", {
            detail: { "code": event.code, "alt": event.altKey, "shift": event.shiftKey, "ctrl": event.ctrlKey }}));
